...
export default function QueuePage() {
  ...
  const progress    = queue ? Math.min(100, Math.round(((queue.currentServing ?? 0) / slot.number) * 100)) : 0;

  return (
    <main className="page grid-bg safe-bottom">
      <div className="orb orb-blue"  style={{ width: 500, height: 500, top: -150, left: '50%', transform: 'translateX(-50%)' }} />
        ...
        <div className="container anim-fade-in" style={{ width: '100%' }}>
          ...
          {isCompleted && (
            <div className="card card-pad anim-slide-up" style={{ textAlign: 'center', borderColor: 'rgba(0,245,160,0.35)', marginBottom: 20 }}>
              ...
              <p style={{ color: 'var(--text-secondary)' }}>You've been checked in successfully.</p>
            </div>
          )}
          ...
          {isMissed && (
            <div className="card card-pad anim-slide-up" style={{ textAlign: 'center', borderColor: 'rgba(255,71,87,0.35)', marginBottom: 20 }}>
              ...
              <p style={{ color: 'var(--text-secondary)', marginBottom: 20 }}>Your 10-minute window expired.</p>
              <Link href="/missed" className="btn btn-amber btn-full">Rejoin Queue →</Link>
            </div>
          )}
          ...
          {!isCompleted && !isMissed && (
            <>
              <div className="card card-pad" style={{ marginBottom: 16, textAlign: 'center' }}>
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Now Serving</p>
                <div id="live-serving-number" className="queue-number queue-number-neon">#{queue?.currentServing ?? '—'}</div>
              </div>

              <div className="card card-pad" style={{ marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}>
                <div>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', fontWeight: 600, textTransform: 'uppercase' }}>Your Number</p>
                  <div className="queue-number queue-number-electric" style={{ fontSize: '3.5rem', lineHeight: 1 }}>#{slot.number}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', fontWeight: 600, textTransform: 'uppercase' }}>Ahead of you</p>
                  <p style={{ fontSize: '2rem', fontWeight: 800 }}>{aheadCount}<span style={{ fontSize: '1rem', fontWeight: 500, color: 'var(--text-muted)', marginLeft: 4 }}>people</span></p>
                </div>
              </div>

              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 6 }}>
                  <span>Entry progress</span><span>{progress}%</span>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                <div className="card card-pad-sm" style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--amber)' }}>{waitMins !== null ? `~${waitMins}m` : '—'}</div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 4 }}>EST. WAIT</div>
                </div>
                <div className="card card-pad-sm" style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--danger)' }}>{Math.max(0, Math.round((slot.expiresAt.getTime() - Date.now()) / 60000))}m</div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 4 }}>EXPIRES IN</div>
                </div>
              </div>

              <div className="alert alert-info" style={{ textAlign: 'center', fontSize: '0.85rem' }}>🔔 You'll receive a push notification when your turn is near</div>
            </>
          )}
        </div>
      </div>
